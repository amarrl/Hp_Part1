...
public class Swipe_till_Element implements Nlp {
	...
	public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
		...
		try
		{
			if (androidDriver!=null) {
				...
				int endPoint = (int) (screenSize.getWidth()*0.5);//Identify ending point of scroll
				int count = 0;	
				...
				while (count<=maxSwipeCount) {
					...
					swipe.addAction(finger.createPointerMove(Duration.ofMillis(700), PointerInput.Origin.viewport(), endPoint, start));
				...
				int screenCenter = (int) (screenSize.getWidth()*0.5);//Identify center point of screen for Y axis
				int startPoint = (int) (screenSize.getWidth()*0.8);//Identify beginning point of scroll for X axis
				int endPoint = (int) (screenSize.getWidth()*0.5);
				int count = 0;	
